...
PonyClaw's tools configuration is located in the `tools` field of `config.json`.
...
By default, PonyClaw blocks the following dangerous commands:
...
All configuration options can be overridden via environment variables with the format `PONYCLAW_TOOLS_<SECTION>_<KEY>`:

For example:
- `PONYCLAW_TOOLS_WEB_BRAVE_ENABLED=true`
- `PONYCLAW_TOOLS_EXEC_ENABLE_DENY_PATTERNS=false`
- `PONYCLAW_TOOLS_CRON_EXEC_TIMEOUT_MINUTES=10`
